...
} from "@/lib/organizer-registration-schema"
...
export function OrganizerRegistration() {
  ...
  const [selectedSkills, setSelectedSkills] = useState<OrganizerSkillId[]>([])
  ...
  const updateSelectedSkill = (skill: OrganizerSkillId, selected: boolean) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const validation = organizerRegistrationSchema.safeParse(submission)
    ...
    try {
      ...
      router.replace("/2026/organizer/success")
    } catch (error) {
      toast.error(
